...
XiaoMai incorporates a comprehensive suite of Markdown extension components adhering to Material 3 Expressive standards. All components are statically rendered via AST transformers at build time, ensuring zero runtime layout shifts and minimal client payload.
...
== JavaScript
```javascript
console.log("Hello, XiaoMai!");
```

== TypeScript
```typescript
const greeting: string = "Hello, XiaoMai!";
...
XiaoMai provides responsive, lazy-loaded video embedding directives for major platforms and custom video playback:
