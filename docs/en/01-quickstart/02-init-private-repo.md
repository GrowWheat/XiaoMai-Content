...
If you have already cloned the theme code repository (e.g., `D:\Code\XiaoMai`), using the built-in decoupling wizard provides the cleanest, safest initialization.

### Why Choose `content:eject`?
- **Automated Directory Assembly**: Automatically extracts posts, moments, albums, and data entities, generating a compliant `xiaomai.content.json` manifest and GitHub Actions trigger workflow;
...
# 2. Confirm and execute export (defaults to ../xiaomai-content in parent directory)
...
3. Fill in repository settings:
   - **Repository name**: e.g., `my-blog-content` or `xiaomai-content`;
...
git clone https://github.com/GrowWheat/XiaoMai-Content.git my-blog-content
...
└── xiaomai.content.json     # Content repository protocol manifest
```
