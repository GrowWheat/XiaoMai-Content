...
XiaoMai provides a dedicated suite of CLI tools tailored for the Content Separation Architecture. Whether writing in external editors, executing preflight validation on YAML configurations, or performing bidirectional synchronization, each action is handled via concise commands.
...
- Generates a compliant `xiaomai.content.json` manifest and GitHub Actions trigger workflow;
