...
1. **Theme Code Repository** (e.g., `D:\Code\XiaoMai`);
...
Beyond `content:sync` and `content:watch`, XiaoMai provides in-memory preflight validation (`content:validate`), differential export (`content:export`), safe cleanup (`content:clean`), and connection inspection (`content:status`).
...
Verify that `$env:CONTENT_DIR` points to a valid absolute path containing `config/`, `content/`, and `xiaomai.content.json`.
