...
Adhering to the [llms.txt specification](https://llmstxt.org/) and the Zero Extra Burden Principle, XiaoMai statically generates `/llms.txt` (concise index) and `/llms-full.txt` (full compiled content) at build time. This provides structured Markdown indices for LLM discovery without impacting reader experience or adding client-side JS overhead.
