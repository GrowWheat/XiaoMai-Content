...
XiaoMai provides a data-driven sidebar layout system supporting single-column or dual-column layouts on wide displays, with granular control over sticky behavior and page routing filters.
...
XiaoMai features a Material 3 Expressive desktop context menu with zero extra burden: when disabled, zero DOM footprint, zero client runtime listeners, and zero external requests are produced.
