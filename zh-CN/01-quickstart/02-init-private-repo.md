...
如果你已经拥有主题代码仓库（例如 `D:\Code\XiaoMai`），使用主题内置的解耦向导是最优雅、最省心的初始化方式。

### 为什么推荐 `content:eject`？
- **全自动结构组装**：自动提取文章、动态、相册、数据实体，并生成合规的 `xiaomai.content.json` 清单与 GitHub Actions 自动化触发工作流；
...
# 2. 确认无误后执行迁出（默认导出至上一级目录的 ../xiaomai-content）
...
3. 填写仓库信息：
   - **Repository name**：建议命名为 `my-blog-content` 或 `xiaomai-content`；
...
└── xiaomai.content.json     # 内容仓库元数据标识文件
```
