...
确保电脑上已有两个本地文件夹：
1. **主题代码仓库**（例如本地路径为 `D:\Code\XiaoMai`）；
...
除了 `content:sync` 和 `content:watch` 之外，XiaoMai 还提供了内存预检（`content:validate`）、差异导出（`content:export`）、安全清理（`content:clean`）以及状态查询（`content:status`）等丰富工具。
